...
public class AchievementServiceImplDb implements AchievementService {
    ...
    public <T extends Achievement> Collection<T> getAchievementsWithProgress(int userId) throws SQLException {
        Connection connection = DBConnector.getConnection();
        PreparedStatement statement = connection.prepareStatement("SELECT a.id, a.type, a.required_value, a.description, ua.value FROM achievements as a LEFT JOIN user_actions as ua on a.type = ua.type AND ua.user_id = ?;");
        statement.setInt(1, userId);
        ResultSet set = statement.executeQuery();
        Collection<T> achievements = new ArrayList<>();
        while (set.next()){
