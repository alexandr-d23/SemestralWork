...
<div class="my_container">
    ...
    <div class="my-center-block">
        <p class="no_money_notification">${requestScope.get('description')}</p>
        <p class="bought_item">${requestScope.get('bought')}</p>
        <c:forEach var="item" items="${requestScope.get('items')}">
            <t:shot_item id="${item.id}" description="${item.description}" cost="${item.cost}"/>.
        </c:forEach>
    </div>
...
<t:footer footerText="Текст в футере Стартовой страницы"/>
